...
- Pq min heap using Functor logic:
#include <queue>
#include <vector>

struct MinHeap {
    bool operator()(const int& a, const int& b) const {
        return a > b; // makes the smallest element the "top"
    }
};
...
Before the type → protects the thing being passed/returned (const int& x).

After the function → promises "this function won’t change the object."

[*] heaps
- for heaps see if you could use min heap rather than max, this might save a lot of time meaning O(n log n ) can be done in O(n log k) sometimes!
...
## C++ Type Conversions (int, char, string, char*)
